package com.sample.demo.urlShortener.util;
...
public class Base62 {
    ...
    public String addPadding(String decodedValue){
        if(decodedValue.length() < 10){
            System.out.println("Add padding - decodedValue :"+decodedValue);
            while(decodedValue.length()<10){
                decodedValue = "0".concat(decodedValue);
            }
            System.out.println("Add padding - Padded value :"+decodedValue);
        }
        return decodedValue;
    }
